...
// AddReservedAuthorizationBlock creates a Prism block containing
// signed reserved-supply authorizations.
//
// The complete candidate chain is validated before the local
// blockchain is mutated.
func (bc *Blockchain) AddReservedAuthorizationBlock(
	authorizations []reserved.Authorization,
	proposer string,
	...
	if len(authorizations) == 0 {
		return Block{}, fmt.Errorf(
			"reserved authorization block cannot be empty",
		)
	}
	...
	block := Block{
		...
		ReservedAuthorizations: blockAuthorizations,
	}
	...
	if !candidate.ValidateChain(pos) {
		return Block{}, fmt.Errorf(
			"invalid reserved authorization block",
		)
	}
